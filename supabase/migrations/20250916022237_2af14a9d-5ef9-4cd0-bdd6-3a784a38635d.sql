...
DROP POLICY IF EXISTS "Group members can view events" ON grupo_eventos;
...
DROP POLICY IF EXISTS "Group moderators can create events" ON grupo_eventos;
...
DROP POLICY IF EXISTS "Event creators can update their events" ON grupo_eventos;
CREATE POLICY "Event creators can update their events" ON grupo_eventos
FOR UPDATE USING (auth.uid() = creado_por);

DROP POLICY IF EXISTS "Event creators can delete their events" ON grupo_eventos;
...
DROP POLICY IF EXISTS "Group members can view event attendees" ON grupo_eventos_asistentes;
...
DROP POLICY IF EXISTS "Users can register for events" ON grupo_eventos_asistentes;
...
DROP POLICY IF EXISTS "Users can update their own attendance" ON grupo_eventos_asistentes;
CREATE POLICY "Users can update their own attendance" ON grupo_eventos_asistentes
FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can cancel their attendance" ON grupo_eventos_asistentes;
